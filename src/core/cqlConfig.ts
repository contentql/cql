...
const cqlConfig = ({
  ...
  if (globals.length) {
    globals.forEach(globalCollection => {
      ...
      if (index !== -1) {
        defaultGlobals[index] = deepMerge(defaultGlobals[index], defaultGlobals)
      }
  ...
  return buildConfig({
    ...
    collections: defaultCollections as CollectionConfig[],
    globals: [siteSettings, ...globals] as GlobalConfig[],
    db: mongooseAdapter({
      url: dbURL,
    }),
    secret,
